...
static bb::cascades::Image fromQImage(const QImage &origQImage,
	...
	for (; y < imageData.height(); y++) {
		unsigned char * dst = dstLine;
		for (int x = 0; x < imageData.width(); x++) {
			QRgb srcPixel = mirroredQImage.pixel(x,
					(y - 2 - origQImage.height()));
			*dst++ = qRed(srcPixel);
			*dst++ = qGreen(srcPixel);
			*dst++ = qBlue(srcPixel);
			*dst++ = qAlpha(srcPixel);
...
QVariant Carousel::createMirrorImage(QString inputFName, int pitch) {
	...
	QTransform Qtrans;
	Qtrans.rotate(pitch, Qt::YAxis).translate(212, -140);
	//QImage transformed_image= inputQImage.transformed(
//		  QImage::trueMatrix(Qtrans,426,280),Qt::SmoothTransformation);
	QImage transformed_image = inputQImage.transformed(Qtrans,
			Qt::SmoothTransformation);

	QImage mirrored_part = transformed_image.mirrored(false, true);
	...
	bb::cascades::Image mirrored_image = fromQImage(transformed_image,
			mirrored_part);
//  bb::cascades::Image mirrored_image = fromQImage(transformed_image);
